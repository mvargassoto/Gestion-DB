...
BEGIN
    ...
    SELECT DISTINCT VENTA_CODIGO, ENVIO_FECHA_PROGAMADA, ENVIO_HORA_INICIO, ENVIO_HORA_FIN_INICIO, ENVIO_FECHA_ENTREGA, ENVIO_COSTO, ENVIO_TIPO
    FROM gd_esquema.Maestra
END
GO
...
AS
BEGIN
    INSERT INTO [EXEL_ENTES].[Localidad] (Descripcion)
...
AS
BEGIN
    INSERT INTO [EXEL_ENTES].[Subrubro] (Descripcion)
...
AS
BEGIN
    INSERT INTO [EXEL_ENTES].[Detalle_Factura] (Tipo, Cantidad, Precio, Subtotal)
    SELECT DISTINCT FACTURA_DET_TIPO, FACTURA_DET_CANTIDAD, FACTURA_DET_PRECIO, FACTURA_DET_SUBTOTAL
    FROM gd_esquema.Maestra
...
BEGIN
    ...
    FROM gd_esquema.Maestra
    WHERE PAGO_CANT_CUOTAS IS NOT NULL
END
GO
